...
public class TagServiceImpl implements ITagService {
    ...
    private final List<AssetProcessHandler> assetProcessHandlerList;

    public TagServiceImpl(List<AssetProcessHandler> processChainList) {
        assetProcessHandlerList = processChainList.stream().sorted(Comparator.comparing(s -> s.getStep().getStep())).collect(Collectors.toList());
    }
    ...
    private void assetProcess(AssetProcessingContext context) {
        try {
            ...
            assetProcessHandlerList.forEach(chain -> {
                //画像情報を読み込む
                //タグを取得
                chain.process(context);
            });
        } catch (Exception e) {
            log.warn("TagServiceImpl assetProcess error [{}]", e.getMessage());
        }
